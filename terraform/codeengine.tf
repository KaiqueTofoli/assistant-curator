...
resource "null_resource" "codeengine" {
  provisioner "local-exec" {
    ...
    environment = {
      ...
      COGNOS_USERNAME         = ibm_resource_key.cognos.credentials.client_id
      COGNOS_PASSWORD         = ibm_resource_key.cognos.credentials.client_secret
      CLOUDANT_API_KEY        = ibm_resource_key.cloudant.credentials.apikey
      CLOUDANT_URL            = ibm_resource_key.cloudant.credentials.url
    }
  }
}
